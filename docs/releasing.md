...
2. Create an exact Semantic Versioning Git tag without a `v` prefix, such as `0.5.0`, at the reviewed commit.
...
The workflow uses only the repository-scoped `GITHUB_TOKEN`. It publishes `0.5.0`, `0.5` and `0` for release `0.5.0`; it never publishes `latest`.
